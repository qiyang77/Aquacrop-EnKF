# -*- coding: utf-8 -*-
"""
Created on Sun Jul 18 17:32:06 2021

@author: Qi Yang

OSS Experiment for only assimilating Canopy Cover (CC)
"""
...
class Aquacrop_env():
    def __init__(self, inputpath, start_date, ensemble_n, init_para, state_case = 1, initUpdate = False):
        ...
        if self.state_case == 1:
            ...
            self.stateList = ['CC', 'biomass']
        elif self.state_case == 3:
            self.stateList = ['CC', 'biomass', 'Para_CDC', 'Para_CGC','Para_CCx']
            ...
            self.stateList = ['CC', 'biomass', 'Para_CDC', 'Para_CGC','Para_CCx', 'Para_WP','Para_Zmax','Para_p_up2','Para_p_up4'
                              , 'Para_Tbase','Para_HIstart','Para_YldForm', 'Para_Senescence']
...
if __name__ == '__main__':
    ...
    ##  model settings
    inputpath = './AquaCropOS_v60a/Input/rice_NN/'   # Nanning station
    start_date = ['2039','7','15']    # normal year  
    ensemble_n = 300
    np.random.seed(0)
    OSS_path='OSSE_truth'
    outPath = 'OSSresult_1obs'#'OSSresult_1obs'
    saveResult = False#True
    forceOpenloop = True#False
    ...
    # load the "ground truth" of OSS experiment which was generated by one random run 
    z_origin = np.loadtxt('%s/sim_z.in'%OSS_path)
    ...
    for case_n, state_case in enumerate(state_case_list):
        ...
        R = np.diag([0.03**2])  # measurement cov matrix, for Canopy Cover
        P0 = np.diag([0.])
       
        if case_n == 0: 
            if (not os.path.exists('openLoopResult.npy')) | forceOpenloop:
                # ENKF - open loop
                DAA_enkf = EnKF(x=np.zeros(len(Aquacrop.stateList)), P=P0, dim_z=1, N=ensemble_n, hx = hx, fx = Aquacrop.steprun, x_paraLoc = x_paraLoc)
                DAA_enkf.R = R
                dt = 1
                xs_enkf = []
                sigmas_enkf = []
        ...
        while not DAA_enkf.allModelDone:
            ...
            # update
            if DAP in z_DAP:
                DAA_enkf.update(np.asarray([z[ass_n]]))
        ...
        # plot CC
        # fig = DA_plot_state(DA_state_loc = 0, y_trueth = z_origin, ylabel = 'Canoy cover %', title = 'Case %d'%state_case)
        ...
        # plot yield
        y_trueth = np.loadtxt('%s/sim_yield.in'%OSS_path)
        ...
        # plot Biomass   
        y_trueth = np.loadtxt('%s/sim_bio.in'%OSS_path)
        ...
        if state_case >= 5:
            # plot GDD
            y_trueth = np.loadtxt('%s/sim_GDD.in'%OSS_path)
