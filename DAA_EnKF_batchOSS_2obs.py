# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 17:20:42 2021

@author: Qi Yang

OSS Experiment for assimilating Canopy Cover (CC) and aboveground biomass
"""
...
if __name__ == '__main__':
    ...
    start_date = ['2039','7','15']       
    ensemble_n = 300
    np.random.seed(0)
    OSS_path='OSSE_truth'
    outPath = 'OSSresult_2obs'
    saveResult = True
    ...
    z2_origin = np.loadtxt('%s/sim_bio.in'%OSS_path)
    ...
    for case_n, state_case in enumerate(state_case_list):
        ...
        R = np.diag([0.03**2, 50.0**2])  # measurement cov matrix
        P0 = np.diag([0.])
       
        if case_n == 0: 
            # ENKF - open loop
            DAA_enkf = EnKF(x=np.zeros(len(Aquacrop.stateList)), P=P0, dim_z=2, N=ensemble_n, hx = hx, fx = Aquacrop.steprun, x_paraLoc = x_paraLoc)
            DAA_enkf.R = R
            dt = 1
            xs_enkf = []
            sigmas_enkf = []
        ...
        while not DAA_enkf.allModelDone:
            ...
            # update
            if DAP in z_DAP:
                DAA_enkf.update(np.asarray([z[ass_n],z2[ass_n]]))
